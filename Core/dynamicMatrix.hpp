...
public:
    ...
    DynamicMatrix() : data(1, std::vector<T>(1, T(0))) {}  // Default constructor with one element
    // Constructor that creates a matrix with n rows and m columns, initialized to zero
    DynamicMatrix(int rows, int cols) : data(rows, std::vector<T>(cols, T(0))) {}
    ...
    struct IdentityMatrixCreator;

    //DynamicMatrix<T> diagonalBlock(DynamicMatrix<T> lhs,DynamicMatrix<T> rhs);
    
    // template <typename U = T>
    // struct IdentityMatrixCreator {
    //     static DynamicMatrix<U> create(size_t rows, size_t cols) {
    //         DynamicMatrix<U> identityMatrix(rows, cols);
